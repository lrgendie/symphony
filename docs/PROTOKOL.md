...
| `GET /api/context-map?limit=<n>&week=<YYYY-Www>&flat=1` | Bearer | Bağlam haritası grafı (ADR-016 Karar 6 + ADR-019 Karar 2/3/4, Dilim H2): `{ nodes: [{id, kind, label, at, meta}], edges: [{from, to, kind}] }` — mevcut sessions/agent_runs verisinden deterministik türetim + kalıcı kürasyon (`map_nodes`/`map_edges`) bindirmesi, vars. son 500 düğüm. **ADDITIVE + gevşek (ADR-019 Karar 7b):** `kind` alanları artık `z.string()` (eski istemci katı enum'la kırılmasın diye) — bilinen düğüm türleri `session\|run\|project\|context\|group\|week\|model\|agent`, kenar türleri `project\|same_day\|pin\|link\|member\|model\|agent\|week`. İçinde bulunulan ISO haftası DIŞINDAKİ ve sabitlenmemiş (bir `context` düğümünün ref'lemediği) session/run'lar tek tek dönmez, hafta başına tek `week:<label>` düğümüne katlanır (ADR-019 Karar 4); `week=<YYYY-Www>` verilirse o hafta AÇIK döner (drill-down), `flat=1` katlamayı tamamen kapatır (geri dönüş anahtarı) |
...
| `doctor.diagnose` | `{}` | Kendini geliştirme (ADR-018 Karar 1): tekrarlayan hata adayları. Cevap: `doctor.diagnose.ok { candidates: [{ code, count }] }`. **Deterministik** — eşik tabanlı (`selfDev.minRecurrence` / `windowDays`), LLM'e "hangi hata önemli" SORULMAZ; hâlâ açık (`proposed`) ya da uygulanmış (`applied`) yaması olan kodlar elenir |
| `patches.list` | `{}` | Yama önerileri (ADR-018 Karar 3): `{ patches: PatchSummary[] }` — `diff` BİLİNÇLE taşınmaz (büyük olabilir) |
| `patch.resolve` | `{ patchId, state: "applied"\|"rejected"\|"reverted"\|"failed" }` | Bir yamanın durumunu YAZAR. **Uygulamanın kendisi burada DEĞİL:** merge → build+test → shutdown → restart → sağlık → (bozuksa) geri alma zinciri CLI'nin `symphony patch apply` komutundadır (ADR-018 Karar 3 — "restart'ı daemon'ın içinden yönetmek kendi bacağını kesmektir"). Daemon yalnız sonucu kaydeder |
| `map.pin` | `{ title?, ref?: { kind: "session"\|"run", id } }` | Bağlam Haritası kürasyonu (ADR-019 Karar 1/2, Faz "H" Dilim H1): "bunu haritaya ekleyelim" anı — kalıcı bir `context` düğümü doğurur. **`ref` verilmezse `title` ZORUNLUDUR** (şema seviyesinde); `ref` verilirse başlık verilmezse oturum başlığından/koşu görevinden türetilir (türetilen başlık boşsa `"(adsız)"`, Y4). `ref` bilinmeyen bir session/run'a işaret ediyorsa `VALIDATION_MAP_REF_UNKNOWN`. **Aynı `ref` zaten sabitlenmişse yeni düğüm ÜRETİLMEZ, mevcut `nodeId` döner** (Y5, idempotent). Cevap: `map.pin.ok { nodeId }` |
| `map.node.rename` | `{ nodeId, title }` | Yalnız GERÇEK bir kürasyon düğümünü (context/group) yeniden adlandırır. Türetilmiş/gerçek bir graf öğesiyse (proje/model/agent/hafta ya da session/run) `VALIDATION_MAP_NODE_PROTECTED`; hiçbir karşılığı yoksa `VALIDATION_MAP_NODE_UNKNOWN`. Cevap: `map.node.rename.ok {}` |
| `map.node.delete` | `{ nodeId }` | Kürasyon düğümünü VE ona değen tüm kenarları siler (uygulama-içi kaskad, `map_edges`'te FK yok). Aynı PROTECTED/UNKNOWN kuralı `map.node.rename` ile aynıdır. Cevap: `map.node.delete.ok {}` |
| `map.group.create` | `{ title, members: string[] }` | Yeni bir `group` kürasyon düğümü + her üye için `member` kenarı (`from: üye, to: grup`). Her üye id'si kürasyon düğümü, türetilmiş id ya da gerçek session/run olmalı — biri değilse `VALIDATION_MAP_NODE_UNKNOWN`. **`members` içindeki tekrarlar tek kenara indirgenir** (Y5, `new Set`). Cevap: `map.group.create.ok { nodeId }` |
| `map.member.add`\|`map.member.remove` | `{ groupId, nodeId }` | Bir düğümü gruba ekler/gruptan koparır (`member` kenarı). `add`de `groupId` GERÇEK bir `group` düğümü olmalı (değilse `VALIDATION_MAP_NODE_UNKNOWN`), `nodeId === groupId` ise `VALIDATION_MAP_SELF_MEMBER` (Y5 — bir düğüm kendi grubuna üye olamaz), tekrar ekleme çoğaltmaz; `remove` HER ZAMAN güvenlidir (eşleşme yoksa no-op). Cevap: `map.member.add.ok {}` / `map.member.remove.ok {}` |
| `map.link.add`\|`map.link.remove` | `{ from, to }` | İki düğüm arasında serbest bir `link` kenarı kurar/koparır. `add`de iki uç da bilinen bir düğüme (kürasyon/türetilmiş/gerçek session-run) işaret etmeli, değilse `VALIDATION_MAP_NODE_UNKNOWN`; `from === to` ise `VALIDATION_MAP_SELF_LINK` (Y5 — kendine bağ anlamsız); `remove` HER ZAMAN güvenlidir. Cevap: `map.link.add.ok {}` / `map.link.remove.ok {}` |
| `doctor.run` | `{ errorCode?, proje? }` | Doktor boru hattını başlatır (ADR-018 Karar 2): sandbox (git worktree + `doktor/<slug>` dalı + `pnpm install`) aç → teşhis dosyasını yaz → `doktor` agent'ını koştur → **boru hattı** build/test/lint koşar → değişiklikleri dalda commit'le → yama önerisi kaydet. Cevap yalnız bir ACK'tir: `doctor.run.ok {}` — **runId TAŞIMAZ**, çünkü ilk adım (worktree + install) tek başına dakikalarca sürer ve WS istek zaman aşımına (30sn) sığmaz; runId agent koşusu doğunca `doctor.phase`/`agent.run.started` ile gelir. İlerleme `doctor.phase`, sonuç `doctor.patch.proposed` olaylarıyla akar. Repo yolu yoksa `VALIDATION_SELFDEV_REPO_REQUIRED`, kod penceredeki telemetride yoksa `VALIDATION_DOCTOR_CODE_UNKNOWN`, başka bir doktor koşusu sürüyorsa `AGENT_DOCTOR_BUSY`. **ADDITIVE (Faz 8 Dilim D6):** `proje` verilirse `errorCode` YOK SAYILIR — bekçi projesi modu: sandbox `bekci.json`daki `repoPath`'ten açılır, kod `BEKCI_<AD>` daemon'ca türetilir, doğrulama sabit `pnpm build/test/lint` yerine o projenin `testCommand`'ıyla yapılır (yoksa `test_ok:false` + dürüst bir özetle atlanır — asla sessizce geçmez). Proje kayıtlı değilse `VALIDATION_BEKCI_PROJECT_UNKNOWN`, hiç bekçi kaydı yoksa `VALIDATION_BEKCI_NO_EVIDENCE` |
...
| `log.entry` | `{ level, source, message, runId? }` | Canlı log akışı (UI log paneli). **İlk üreticisi (Faz 8, Dilim D5):** daemon'ın günlük tekrarlayan-hata taraması — `doctor.diagnose()` aday bulursa `source: "doctor"`, `level: "warn"` ile yayınlanır. **İkinci üretici (Dilim D6):** bekçi log izleyicisi — kayıtlı bir projenin log dosyasında hata deseni yakalarsa `source: "bekci:<ad>"`, `level: "warn"` ile yayınlanır (5 dk debounce) |
