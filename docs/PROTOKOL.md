...
| `map.pin` | `{ title?, ref?: { kind: "session"\|"run", id } }` | Bağlam Haritası kürasyonu (ADR-019 Karar 1/2, Faz "H" Dilim H1): "bunu haritaya ekleyelim" anı — kalıcı bir `context` düğümü doğurur. **`ref` verilmezse `title` ZORUNLUDUR** (şema seviyesinde); `ref` verilirse başlık verilmezse oturum başlığından/koşu görevinden türetilir. `ref` bilinmeyen bir session/run'a işaret ediyorsa `VALIDATION_MAP_REF_UNKNOWN`. Cevap: `map.pin.ok { nodeId }` |
...
| `map.group.create` | `{ title, members: string[] }` | Yeni bir `group` kürasyon düğümü + her üye için `member` kenarı (`from: üye, to: grup`). Her üye id'si kürasyon düğümü, türetilmiş id ya da gerçek session/run olmalı — biri değilse `VALIDATION_MAP_NODE_UNKNOWN`. Cevap: `map.group.create.ok { nodeId }` |
| `map.member.add`\|`map.member.remove` | `{ groupId, nodeId }` | Bir düğümü gruba ekler/gruptan koparır (`member` kenarı). `add`de `groupId` GERÇEK bir `group` düğümü olmalı (değilse `VALIDATION_MAP_NODE_UNKNOWN`), tekrar ekleme çoğaltmaz; `remove` HER ZAMAN güvenlidir (eşleşme yoksa no-op). Cevap: `map.member.add.ok {}` / `map.member.remove.ok {}` |
| `map.link.add`\|`map.link.remove` | `{ from, to }` | İki düğüm arasında serbest bir `link` kenarı kurar/koparır. `add`de iki uç da bilinen bir düğüme (kürasyon/türetilmiş/gerçek session-run) işaret etmeli, değilse `VALIDATION_MAP_NODE_UNKNOWN`; `remove` HER ZAMAN güvenlidir. Cevap: `map.link.add.ok {}` / `map.link.remove.ok {}` |
