...
| `simple-git` | Agent'ın git işlemleri + oturum yedekleme. **ADR-017 (Faz 7, Dilim F4):** `symphony sync` bunu kullanır — `cli` paketine ✅ kuruldu 2026-07-11 (`~/.symphony` içi git repo, beyaz listeli eşitleme). **ADR-018 (Faz 8, Dilim D2):** doktor sandbox'ı (git worktree + dal + commit) — `core` paketine de ✅ kuruldu 2026-07-11 |
